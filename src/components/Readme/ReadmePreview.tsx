...
const ReadmePreview: React.FC<ReadmePreviewProps> = ({ readme }) => {
  ...
  // Scroll to bottom when animatedReadme changes
  useEffect(() => {
    if (bottomRef.current && !hasScrolled) {
      bottomRef.current.scrollIntoView({ behavior: 'smooth' });
      setHasScrolled(true);
    }
  }, [animatedReadme, hasScrolled]);
  ...
  return (
    <div
      className="mt-8 border-2 border-cyan-400/10 rounded-lg p-4 w-full 
